...
(function () {
	...
	const buttonLabels = ["pay me", "book me", "donate", "support me"];
	let buttonColor = "#ffffff";
	let backgroundColor = "#292a73";
	...
	for (let i = 0; i < templateFidiaButton.length; i++) {
		...
		newFidiaButton.setAttribute("data-fidia-label", currentButton.getAttribute("data-fidia-label"));

		newFidiaButton.setAttribute("fidia-button-color", currentButton.getAttribute("fidia-button-color"));
		newFidiaButton.setAttribute("fidia-button-background", currentButton.getAttribute("fidia-button-background"));

		if (currentButton.getAttribute("fidia-button-background")) {
			backgroundColor = currentButton.getAttribute("fidia-button-background");
		}
		
		if (currentButton.getAttribute("fidia-button-color")) {
			buttonColor = currentButton.getAttribute("fidia-button-color");
		}

		newFidiaButton.style.color = buttonColor;
		newFidiaButton.style.backgroundColor = backgroundColor;

		let buttonLabel = newFidiaButton.getAttribute("data-fidia-label");
		if (!buttonLabels.includes(buttonLabel.toLocaleLowerCase())) {
		...
		newFidiaButton.addEventListener("click", (e) => {
			...
			const fidiaPaymentIframe = d.querySelector("#fidia-embed-iframe");
			setTimeout(() => {
				fidiaPaymentIframe.style.display = "block";
			}, 30);

			// Child window and parent window can only communicate over events
			fidiaPaymentIframe.contentWindow.postMessage(fidiaEmbedData, "*");
		});
	}
